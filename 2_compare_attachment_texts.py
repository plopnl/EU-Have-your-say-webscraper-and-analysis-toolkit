    ...
    try:
        if filepath.lower().endswith(".pdf"):
            ...
            if not zipfile.is_zipfile(filepath):
                # EU portal sometimes serves PDFs with a .docx extension — try PDF parser
                with pdfplumber.open(filepath) as pdf:
                    text_out = "\n".join([page.extract_text() or "" for page in pdf.pages])
                print(f"ℹ️  {os.path.basename(filepath)}: treated as PDF (mislabeled .docx)")
                return text_out
...
if not files:
    ...
    ]).to_csv(ATTACHMENT_DUP_FILE, index=False)
    pd.DataFrame(columns=[
        "feedbackId","filename","clean_filename","char_count","word_count"
    ]).to_csv(ATTACHMENT_UNIQUE_FILE, index=False)
    raise SystemExit(0)
...
df_duplicates.to_csv(ATTACHMENT_DUP_FILE, index=False)
...
df_unique.to_csv(ATTACHMENT_UNIQUE_FILE, index=False)
