...
# =========================
df_short = df[df["word_count_original"] <= SHORT_WORD_LIMIT]
df_main = df[df["word_count_original"] > SHORT_WORD_LIMIT]
...
# Ensure empty frames still have the required column
for df_tmp in [df_short_duplicates_original, df_short_duplicates_translated,
               df_duplicates_original, df_duplicates_translated]:
    if df_tmp.empty:
        df_tmp["feedbackId"] = pd.Series(dtype=int)
...
# Original duplicates
df_duplicates_original.to_csv(os.path.join(DATA_DIR, "feedback_duplicates.csv"), index=False)
print(f"✅ Saved {len(df_duplicates_original)} duplicates (original text) → feedback_duplicates.csv")

# Translated duplicates
df_duplicates_translated.to_csv(os.path.join(DATA_DIR, "feedback_duplicates_translated.csv"), index=False)
print(f"📄 Saved {len(df_duplicates_translated)} duplicates (translated text) → feedback_duplicates_translated.csv")

# Short duplicates
df_short_duplicates_original.to_csv(os.path.join(DATA_DIR, "feedback_short_duplicates.csv"), index=False)
...
df_short_duplicates_translated.to_csv(os.path.join(DATA_DIR, "feedback_short_duplicates_translated.csv"), index=False)
...
df_short_unique.to_csv(os.path.join(DATA_DIR, "feedback_short_unique.csv"), index=False)
print(f"📄 Saved {len(df_short_unique)} short unique entries to {DATA_DIR}/feedback_short_unique.csv")
...
df_short_unique_translated.to_csv(os.path.join(DATA_DIR, "feedback_short_unique_translated.csv"), index=False)
...
df_unique.to_csv(os.path.join(DATA_DIR, "feedback_unique.csv"), index=False)
print(f"✅ Saved {len(df_unique)} unique entries to {DATA_DIR}/feedback_unique.csv")
...
df_unique_translated.to_csv(os.path.join(DATA_DIR, "feedback_unique_translated.csv"), index=False)
print(f"✅ Saved {len(df_unique_translated)} unique entries (translated-based) → feedback_unique_translated.csv")


# All similarity scores
df.to_csv(os.path.join(DATA_DIR, "feedback_all_similarity_scores.csv"), index=False)
print(f"📦 Saved {len(df)} total entries to {DATA_DIR}/feedback_all_similarity_scores.csv (with similarity column)")
